...
$(document).ready(function () {
    ...
    $.ajax({
            ...
            success: function (response) {
                product.forEach(function (productUnit, idx) {
                    if (idx > 0) {
                        edit_field(response.data, productUnit);
                        $(`#unit_id_${room}`).val(productUnit.unit_id);
                    }
                });
            },
            error: function (xhr) {
...
function edit_field(units, productUnit) {
    room++;
    var objTo = document.getElementById("education_fields");
    var divtest = document.createElement("div");
    divtest.setAttribute("class", "mb-3 removeclass" + room);
    ...
    selectHTML += `
        ...
        <div class="col-sm-3">
            ...
            <input type="number" value="${productUnit.quantity_in_small_unit}" class="form-control" id="Age"
            name="quantity_in_small_unit[]" placeholder="10" />
    </div>
        </div>
        <div class="col-sm-3">
